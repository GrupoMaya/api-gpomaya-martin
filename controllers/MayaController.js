...
module.exports = {
  ...
  addProyecto: async (req, res) => {
    ...
    try {
      ...
      if (Object.values(proyectoExist).length > 0) {
        throw new Error('Ya existe el documento')
      }
  ...
  findCliente: async ({ query }, res) => {
    try {
      ...
      if (!payload) {
        throw new Error('No hay información con el parametro de busqueda')
      }
  ...
  getPagosByProject: async (req, res) => {
    ...
    try {
      ...
      if (getPagos.lenth === 0) {
        throw new Error('No hay pagos con la infomacion proporcionada')
      }
  ...
  createInvoice: async (req, res) => {
    ...
    try {
      ...
      if (getSettings[0].slug === 'martin') {
        ...
        getPDFdata = await MayaService.createInvoice(
          req.body,
          req.query,
          getSettings
        )
  ...
  findUser: async (req, res) => {
    ...
    try {
      ...
      if (!getUsers.length > 0) {
        throw new Error('No hay usuarios relacionados con tu busqueda')
      }
  ...
  pagosRecords: async (_req, res) => {
    try {
      ...
      const pagosRecords = await Promise.all(
        allClients.map(async (client) =>
          MayaService.pagosRecorsByClient(client._id)
        )
      )
      if (!pagosRecords) throw new Error('No hay pagos')

      const records = pagosRecords
        .flat(Infinity)
        .filter((r) => r.lote_data !== null)

      const allReadyPayments = records.filter((record) =>
        MayaService.inspectPayments(record)
      )
      if (allReadyPayments.length > 0) {
        await Promise.all(
          allReadyPayments.map(async ({ lote_data }) =>
            MayaService.updateLoteStatusIfPaid(lote_data._id, true)
          )
        )
      }

      await MayaService.insertManyPayments(
        records.map((record) => {
          return {
            ...record,
